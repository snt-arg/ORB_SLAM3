...
def save_results_txt(poses: np.ndarray, out_file):
    Path(out_file).parent.mkdir(exist_ok=True, parents=True)
    with open(out_file, 'w') as f:
        for pose in poses:
            if pose.size in [12, 16]:
                pose = pose.ravel()[:12]
            elif pose.size == 7:
                rot = R.from_quat(pose[[4, 5, 6, 3]]).as_matrix()
                pose = np.hstack((rot, pose[:3].reshape(3, 1))).ravel()
            else:
                raise ValueError('Pose input size not recognized.')
            pose_str = np.array2string(pose, precision=7, suppress_small=False,
                                       max_line_width=1000, separator=' ', prefix='', suffix='')[1:-1].strip()
            pose_str = ' '.join(x.strip() for x in pose_str.split(' ') if x.strip() != '')
            f.write(pose_str)
            f.write('\n')
...
class KittiEvalOdom():
    ...
    def __init__(self, lengths=None, step_size=10):
        if lengths is None:
            lengths = [100, 200, 300, 400, 500, 600, 700, 800]  # kitti default
        elif type(lengths) in [tuple, list] and len(lengths) == 3:
            lengths = np.linspace(lengths[0], lengths[1], lengths[2]).astype(np.int)
        else:
            lengths = lengths
    ...
    def eval(self, gt_dir, result_dir,
        ...
        error_dir = "error_dir"
        self.plot_path_dir = "plot_path"
        self.plot_error_dir =  "plot_error"
        ...
        return [[ave_t_errs[i] * 100 for i in range(len(ave_t_errs))],
                [ave_r_errs[i] / np.pi * 180 * 100 for i in range(len(ave_t_errs))],
                seq_ate, seq_rpe_trans,
                [seq_rpe_rot[i] * 180 / np.pi for i in range(len(ave_t_errs))]]
    ...
    def calc_sequence_errors(self, poses_gt, poses_result):
        ...
        for first_frame in range(0, len(poses_gt), self.step_size):
            for i in range(self.num_lengths):
                ...
                speed = self.step_size * len_ / num_frames
    ...
    def plot_trajectory(self, poses_gt, poses_result, seq):
        ...
        poses_dicts = {}
        poses_dicts["Ground Truth"] = poses_gt
        poses_dicts["Ours"] = poses_result
        ...
        for pose_label, pose_dict in poses_dicts.items():
            poses = [pose_dict[k] for k in sorted(list(pose_dict.keys()))]
            poses = np.stack(poses)
            axis[0].plot(poses[:, 0, -1], poses[:, 1, -1], label=pose_label)
            axis[1].plot(poses[:, 2, -1], poses[:, 1, -1], label=pose_label)
            axis[2].plot(poses[:, 0, -1], poses[:, 2, -1], label=pose_label)
            axis[3].plot(poses[:, 0, -1], poses[:, 1, -1], poses[:, 2, -1], label=pose_label)

        handles, labels = axis[-1].get_legend_handles_labels()
        fig.legend(handles, labels, loc="upper right", bbox_to_anchor=(1, .95))
        plt.rcParams.update({'font.size': fontsize_})

        for ax, xl, yl in zip(axis, list('XZXX'), list('YYZY')):
            ax.set_xlabel(xl + ' (m)')
            ax.set_ylabel(yl + ' (m)')
        axis[-1].set_zlabel('Z (m)')
        axis[-1].view_init(elev=90, azim=180)

        fig.set_size_inches(10, 10)
        plt.subplots_adjust(left=.1, bottom=.1, right=.95, top=.95, wspace=.35, hspace=0.25)
